<!-- Repository Name. Preferrably 1-5 words long. -->
<h1 align="center" style="font-weight: bold">
    <span id="min-block-shrink">▀▄▀▄▀▄</span>
    <span id="min-block">inviter</span>
    <span id="min-block-shrink">▀▄▀▄▀▄</span>
</h1>

<p align="center">
    <a href="https://github.com/whinee/inviter/issues">
        <img src="https://img.shields.io/github/issues/whinee/inviter.svg?style=flat-square&logo=data:image/png;base64,">
    </a>
    <a href="https://github.com/whinee/inviter/network/members">
        <img src="https://img.shields.io/github/forks/whinee/inviter.svg?style=flat-square&logo=data:image/png;base64,">
    </a>
    <a href="https://github.com/whinee/inviter/stargazers">
        <img src="https://img.shields.io/github/stars/whinee/inviter.svg?style=flat-square&logo=data:image/png;base64,">
    </a>
    <a href="https://github.com/whinee/inviter/graphs/contributors">
        <img src="https://img.shields.io/github/contributors/whinee/inviter.svg?style=flat-square&logo=data:image/png;base64,">
    </a>
    <a href="https://inviter.whinyaan.xyz/license">
        <img src="https://img.shields.io/badge/LICENSE-A31F34?style=flat-square&logoWidth=25&logo=data:image/png;base64,">
    </a>
</p>
<p align="center">
    <a href="https://app.codacy.com/gh/whinee/inviter/dashboard?branch=master">
        <img alt="Codacy Badge" src="https://img.shields.io/codacy/grade/b55dba11e86b49ba8b51573413e20eac?style=flat-square">
    </a>
</p>
<p align="center">
    <a target="_blank" href="https://discord.com/invite/JbAtUxGcJZ">
        <img src="https://inviter.whinyaan.xyz/JbAtUxGcJZ?theme=transparent&color[serverName]=d64f50&color[header]=9383e2&color[presenceText]=684fe2">
    </a>
</p>

<!-- About section. Preferrably 2-5 sentences long. -->
---

<h4 align="center">
Discord invite embeds!
</h4>

---

Github: [github.com/whinee/inviter](https://github.com/whinee/inviter)

Website: [inviter.whinyaan.xyz](https://inviter.whinyaan.xyz)

---

[github.com/whinee/inviter](https://github.com/whinee/inviter) is a fork of <a target="_blank" href="https://github.com/SwitchbladeBot/invidget">SwitchbladeBot/invidget</a>, aiming to provide more cuztomizeability in regards to the SVGs colors.

To be updated, be sure to watch this repository.

For the legals, visit [LICENSE](license).

Interested in commisioning projects? Inquire through Discord(<a target="_blank" href="https://discord.com/users/867696753434951732">whi_ne | whinyaan.xyz#5135</a>) or through e-mail(<a target="_blank" href="mailto:whinyaan@pm.me">whinyaan@pm.me</a>). Pricing starts at 15 USD.

## **Features**

**inviter** generates SVG renders of Discord invites that can be used in GitHub READMEs or even websites. These are a great way to increase conversions and get more people to join your server.

<!-- Mention examples of application of this repository. -->
## **Usage**

### Dark theme (default)

Format:

```txt
https://inviter.whinyaan.xyz/{code}
```

![preview](https://inviter.whinyaan.xyz/JbAtUxGcJZ)

### Light theme

```txt
https://inviter.whinyaan.xyz/{code}?theme=light
```

![preview](https://inviter.whinyaan.xyz/JbAtUxGcJZ?theme=light)

### Different language

```txt
https://inviter.whinyaan.xyz/{code}?language=pt
```

inviter uses [ISO 639-1 code](https://en.wikipedia.org/wiki/ISO_639-1) for identifying languages.

![preview](https://inviter.whinyaan.xyz/JbAtUxGcJZ?language=pt)

### Customize colors

```txt
https://inviter.whinyaan.xyz/${code}?color[serverName]=d64f50&color[header]=9383e2&color[presenceText]=684fe2
```

![preview](https://inviter.whinyaan.xyz/JbAtUxGcJZ?color[serverName]=d64f50&color[header]=9383e2&color[presenceText]=684fe2)

#### Structure

```txt
color
├── joinButtonBackground
│   ├── joinButtonText
│   ├── online
│   ├── members
│   └── badges
│       ├── PARTNERED
│       │   ├── flowerStar
│       │   └── icon
│       └── VERIFIED
│           ├── flowerStar
│           └── icon
├── background
├── serverName
├── header
├── serverIcon
├── acronymText
└── presenceText
```

To modify the color of `flowerStar` badge of `PARTNERED`, use the following query parameter:

```txt
color[badges][PARTNERED][flowerStar]={hex}
```

#### Notes

- The color customization precedes the theme

<!-- License section. Leave unchanged except when updating the year, using a different license, or changing the style altogether. -->
## **License**

### <a target="_blank" href="https://choosealicense.com/licenses/mit/">MIT</a>

Copyright for portions of project [inviter](https://github.com/whinee/inviter) are held by [Switchblade, Github account [SwitchbladeBot](https://github.com/SwitchbladeBot) owner, 2019] as part of project [invidget](https://github.com/SwitchbladeBot/invidget), by [whi_ne, Github account [whinee](https://github.com/whinee) owner, 2022] as part of project [hyaku](https://github.com/hyaku-dl/hyaku), and by [whi_ne, Github account [whinee](https://github.com/whinee) owner, 2022] as part of project [YAMHL](https://github.com/Pirate-Kings/YAMHL).

All other copyright for project [inviter](https://github.com/whinee/inviter) are held by [Github Account [whinee](https://github.com/whinee) Owner, 2022].

Check the [license](license) for more details.

## **Credits**

### MIT Logo

<a target="_blank" href="https://commons.wikimedia.org/wiki/File:MIT_logo.svg">Massachusetts Institute of Technology</a> (vectorized by <a target="_blank" href="https://en.wikipedia.org/wiki/User:Mysid">Mysid</a>, modified by [whinee](https://github.com/whinee)), Public domain, via Wikimedia Commons

### Icons

<a target="_blank" href="https://icons8.com/icon/102502/exclamation-mark">Exclamation Mark</a>, <a target="_blank" href="https://icons8.com/icon/33294/code-fork">Code Fork</a>, <a target="_blank" href="https://icons8.com/icon/85185/star">Star</a>, <a target="_blank" href="https://icons8.com/icon/34095/group">Group</a>, <a target="_blank" href="https://icons8.com/icon/87276/code">Code</a>, and <a href="https://icons8.com/icon/30888/discord">Discord</a> icons by <a target="_blank" href="https://icons8.com">Icons8</a>

<sub>
    <i>
        <b>NOTE:</b> If a reference or source material is not attributed properly or not at all, please kindly message me at Discord: <a target="_blank" href="https://discord.com/users/867696753434951732">whi_ne | whinyaan.xyz#5135</a> or create a pull request so I can properly give credit to their respective authors.
    </i>
</sub>