...
module.exports = class Discord {
    ...
    static getSplashUrl(guildId, splashId) {
        return `${CDN_BASE_URL}/splashes/${guildId}/${splashId}.png?size=4096`
    }

    static fetchSplash(splashURL) {
        return fetch(splashURL).then(res => res.buffer()).then(buffer => buffer.toString('base64'))
    }
}
